# Pure-Python AES-128 core + CBC helpers
# --- AES core (S_BOX, INV_S_BOX, RCON, helpers, key expansion, block encrypt/decrypt) ---
...
def key_expansion(key):
    # key: bytes or list of ints length 16
...
# Public AES-ECB convenience (keeps compatibility)
def aes_encrypt_ecb(key: bytes, plaintext: bytes) -> bytes:
    ...
    for i in range(0, len(data), 16):
        ...
        enc_block = encrypt_block(block, expanded)
...
def aes_decrypt_ecb(key: bytes, ciphertext: bytes) -> bytes:
    ...
    for i in range(0, len(ciphertext), 16):
        ...
        dec_block = decrypt_block(block, expanded)
        decrypted.extend(dec_block)
    return unpad(bytes(decrypted))

# ----------------------
# CBC mode helpers (uses block encrypt/decrypt)
# ----------------------
def xor_bytes(a: bytes, b: bytes) -> bytes:
    return bytes(x ^ y for x, y in zip(a, b))

def aes_encrypt_cbc(key: bytes, plaintext: bytes, iv: bytes) -> bytes:
    """Encrypt plaintext (bytes) using AES-128-CBC with given 16-byte key and 16-byte IV.
       Returns ciphertext bytes (no IV included)."""
    if len(iv) != 16:
    ...
    for i in range(0, len(data), 16):
        ...
        enc_block = encrypt_block(list(x), expanded)
        cbytes = bytes(enc_block)
        ciphertext.extend(cbytes)
        prev = cbytes
    return bytes(ciphertext)

def aes_decrypt_cbc(key: bytes, ciphertext: bytes, iv: bytes) -> bytes:
    """Decrypt AES-128-CBC ciphertext with given key and IV. Returns plaintext bytes."""
    if len(iv) != 16:
        raise ValueError("IV must be 16 bytes")
    if len(ciphertext) % 16 != 0:
    ...
    for i in range(0, len(ciphertext), 16):
        ...
        dec_block = decrypt_block(list(block), expanded)
        p = xor_bytes(bytes(dec_block), prev)
        plaintext.extend(p)
        prev = block
    return unpad(bytes(plaintext))

#py PyInstaller --onefile file_name.py